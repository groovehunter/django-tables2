...
class EtreeAccessor(CellAccessor):
    def __getitem__(self, key):
        pass
...
class XmlBoundRow(BoundRow):
    typ = 'xmlrow'

    def items(self):
        for column in self.table.columns:
            cell = self.record.find(column.name)
            #cell = self.record.find('name')
            try:
                val = cell.text
            except:
                val = 'n/a'

            yield column.name, val
...
class BoundRows:
    ...
    def __iter__(self):
        ...
        if (self.table.data.typ=='xml'):
            for record in self.data:
                yield XmlBoundRow(record, table=self.table)
        else:
            for record in self.data:
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            if self.data.typ == 'xml':
                return XmlBoundRow(record=self.data[key], table=self.table)
            else:
